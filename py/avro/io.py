...
class DatumReader(object):
  ...
  def _read_default_value(self, field_schema, default_value):
    ...
    if field_schema.type == 'null':
      ...
      # 限定死了default值一定是union中排行第一个位置的类型
...
class DatumWriter(object):
  ...
  def write_data(self, writers_schema, datum, encoder):
    # # TODO(wu.ranbo@yottabyte.cn) 并没有处理默认值
    # 所以如果设置了默认值，除非下面包含null的union的情况只是出bug，别的就直接panic了。
    ...
    if writers_schema.type == 'null':
      ...
      # 如果union中没包含null，那么当设置默认值是不会有任何用的，没值时候直接panic了。
      # 包含了null的union的默认值为null的情况，不会panic但是是有bug，
      # 是因为当dict中datum为None，write_union并不能区分是用户根本没有设置这个记录还是值为null，
      # 即：当包含了null的uion设置的default是string或者别的，结果却会变成default是null。
      # 解决方法：这样的问题在这个commit中go版本解决，只能是引入一个自定义的NULL_RECORD来区别两者
      # # TODO(wu.ranbo@yottabyte.cn) 观察看联调时候这里会不会成为问题
